OutsideLive = {
  ...
  initStages: function() {
    ...
    addPerf: function(stageName, artistName, startTime, endTime) {
    //stageName is string
    ...
    var performanceID = Performances.insert({
      artist: artistName,
      startTime: startTime,
      endTime: endTime,
      setList: [],
    ...
    var performance = Performances.findOne({_id: performanceID});
    OutsideLive.checkIfCurrent(performance);
    return performance;
  },

  addSong: function(songName, artistName, genre, mood) {

    var performance = Performances.findOne({artist: artistName});
    console.log(performance);
    console.log(performance._id);
    var stageID = performance.stageID;
    var lastSongID = _.last(performance.setList)
    if(lastSongID) {
      var lastSong = Songs.findOne({_id: lastSongID});
      var lastSongName = lastSong.name;
    } else {
      var lastSongName = "init";
    }

    console.log(lastSongName);
    var song_attrs = {
      name: songName,
      artist: artistName,
      stageId: stageID,
      perfomanceId: performance._id,
      genre: genre,
      mood: mood,
    ...
    var songID = Songs.insert(song_attrs);
    console.log(songID);
    var newSong = Songs.findOne({_id: songID});

    Performances.update({ _id: performance._id}, {
      $push: {
        setList: songID,
    ...
    var currentPerformance = Performances.findOne({_id: performance._id});

    var stage = Stages.findOne({_id: performance.stageID});
    var playedSongIDs = currentPerformance.setList.slice(0, -1).reverse();

    if(Session.get('currentStage')) {
      ...
      Session.set('playedSongIDs', playedSongIDs);
    }

    if(Songs.find({artist: artistName}).count() !== 0) {
